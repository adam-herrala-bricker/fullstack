...
test('create new EH passes expect props', async () => {
  ...
  const titleInput = container.querySelector("input[name='title']") // eslint-disable-line
  const authorInput = container.querySelector("input[name='author']") // eslint-disable-line
  const urlInput = container.querySelector("input[name='url']") // eslint-disable-line
